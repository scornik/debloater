...
const METRIC_LABELS = {
	'frontend.requests': __( 'Requests a page asks for', 'wp-debloat' ),
	'frontend.scripts.count': __( 'Scripts', 'wp-debloat' ),
	'frontend.styles.count': __( 'Stylesheets', 'wp-debloat' ),
	'frontend.head_bytes': __( 'Bytes in <head>', 'wp-debloat' ),
	'frontend.external_hosts': __( 'Other hosts contacted', 'wp-debloat' ),
	'db.autoload_bytes': __( 'Autoloaded data', 'wp-debloat' ),
	'db.revisions': __( 'Post revisions', 'wp-debloat' ),
	'db.transients_expired': __( 'Expired transients', 'wp-debloat' ),
	'cron.events': __( 'Scheduled events', 'wp-debloat' ),
	'admin.notices': __( 'Admin notices', 'wp-debloat' ),
...
const Deltas = ( { measurements } ) => {
	...
	if ( deltas.length === 0 ) {
		return (
			<p className="wpdebloat-field__empty">
				{ __(
	...
	return (
		<table className="wpdebloat-deltas">
			<thead>
				<tr>
					<th scope="col">{ __( 'Measured', 'wp-debloat' ) }</th>
					<th scope="col">{ __( 'Before', 'wp-debloat' ) }</th>
					<th scope="col">{ __( 'After', 'wp-debloat' ) }</th>
					<th scope="col">{ __( 'Change', 'wp-debloat' ) }</th>
				</tr>
			</thead>
			<tbody>
				{ deltas.map( ( delta ) => (
					...
					>
						...
						<td>
							{ delta.direction === 'unknown' && (
								<span className="wpdebloat-deltas__unknown">
									{ delta.reason ||
										__( 'Not measured', 'wp-debloat' ) }
								</span>
							) }
							{ delta.direction !== 'unknown' && (
								<>
									...
									{ delta.percent !== null &&
										delta.percent !== undefined && (
											<span className="wpdebloat-deltas__percent">
												{ ` (${
													delta.percent > 0 ? '+' : ''
												}${ delta.percent }%)` }
											</span>
...
const Progress = ( { run } ) => {
	...
	return (
		<ol className="wpdebloat-progress">
			{ history.map( ( state, index ) => (
				<li
					key={ `${ state }-${ index }` }
					className={ `wpdebloat-progress__step ${
...
const Report = ( { run, scoreBefore, scoreAfter, onDone } ) => {
	...
	if ( rolledBack ) {
		return (
			<div className="wpdebloat-report is-rolled-back">
				<h2>{ __( 'The change was undone', 'wp-debloat' ) }</h2>
				...
				<p className="wpdebloat-report__reassurance">
					<strong>
						{ __( 'Rollback complete.', 'wp-debloat' ) }
					</strong>{ ' ' }
					{ __( 'Previous configuration restored.', 'wp-debloat' ) }
				</p>

				<Button variant="primary" onClick={ onDone }>
					{ __( 'Back to the overview', 'wp-debloat' ) }
				</Button>
			</div>
		);
	}
	...
	return (
		<div className="wpdebloat-report">
			<h2>
				{ sprintf(
			...
			{ ( scoreBefore !== null || scoreAfter !== null ) && (
				<p className="wpdebloat-report__score">
			...
			<h3>{ __( 'Measured before and after', 'wp-debloat' ) }</h3>
			<Deltas measurements={ run.measurements } />

			<p className="wpdebloat-report__note">
				{ __(
					'These are counts, measured on this site before and after the change. WP Debloat does not report time, because it cannot measure yours.',
					'wp-debloat'
				) }
			</p>
...
export const Run = ( { runId, scoreBefore, onDone } ) => {
	...
	if ( ! run ) {
		return (
			<p className="wpdebloat-loading">
				<Spinner /> { __( 'Starting…', 'wp-debloat' ) }
			</p>
		);
	}
	...
	if ( ! run.finished ) {
		return (
			<div className="wpdebloat-run-live">
				<p className="wpdebloat-loading">
					<Spinner /> { run.label }
				</p>
				<Progress run={ run } />
