...
$debloater_post_ids = array();

for ( $i = 0; $i < $debloater_seed['posts']; $i++ ) {
	...
	$debloater_post_ids[] = $post_id;

	for ( $r = 1; $r <= $debloater_seed['revisions_each']; $r++ ) {
		wp_update_post(
			array(
				'ID'           => $post_id,
...
WP_CLI::log( sprintf( 'Created %d posts.', count( $debloater_post_ids ) ) );

WP_CLI::log( 'Trashing some of them…' );

foreach ( array_slice( $debloater_post_ids, 0, $debloater_seed['trashed'] ) as $post_id ) {
	wp_trash_post( $post_id );
}

WP_CLI::log( 'Seeding spam comments…' );

foreach ( range( 1, $debloater_seed['spam_comments'] ) as $n ) {
	wp_insert_comment(
		array(
			'comment_post_ID'      => $debloater_post_ids[ array_rand( $debloater_post_ids ) ],
...
foreach ( range( 1, $debloater_seed['live'] ) as $n ) {
	set_transient( sprintf( 'debloater_fixture_live_%d', $n ), 'still useful', DAY_IN_SECONDS );
}
...
foreach ( range( 1, $debloater_seed['expired'] ) as $n ) {
	$name = sprintf( 'debloater_fixture_stale_%d', $n );
...
foreach ( range( 1, $debloater_seed['orphan_meta'] ) as $n ) {
	$wpdb->insert(
		$wpdb->postmeta,
		array(
			'post_id'    => 900000 + $n,
			'meta_key'   => '_debloater_fixture_orphan',
...
update_option( 'debloater_fixture_cache', str_repeat( 'x', 512000 ), true );
...
add_filter(
	...
	static function ( $schedules ) {
		$schedules['debloater_fixture_30s'] = array(
			'interval' => 30,
...
if ( ! wp_next_scheduled( 'debloater_fixture_sync' ) ) {
	wp_schedule_event( time() + MINUTE_IN_SECONDS, 'debloater_fixture_30s', 'debloater_fixture_sync' );
}

foreach ( range( 1, 15 ) as $n ) {
	$hook = sprintf( 'debloater_fixture_orphan_%d', $n );
...
foreach ( range( 1, 3 ) as $n ) {
	$login = sprintf( 'debloater_fixture_admin_%d', $n );
