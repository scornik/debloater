...
*From `D-0057` below. Violated in `scornik/debloater-registry`, which
carried a copy of it, and fixed there in `67f3e6a`.*
...
*From `D-0065` in `scornik/debloater-pro`. Its cross-tree invariants skipped on
every run the workflow ever made, reporting success, because they were gated on
a secret nobody had configured.*
...
*From `D-0066` below. The packaging job was red on both runners for six commits
while five other jobs stayed green, and had never passed once.*
...
*Not a numbered decision — it lives in `CLAUDE.md`, under Testing conventions.
Found by probe in `phase-19c-2`: renaming the constant left the test green.*
...
*Not a numbered decision — see `scornik/debloater-registry`, `7d73490`, and the
comments in its `pipeline/lib/direction.mjs` and `pipeline/lib/factdiff.mjs`.*
...
*From `67f3e6a` in `scornik/debloater-registry`, extending `D-0057`.*
