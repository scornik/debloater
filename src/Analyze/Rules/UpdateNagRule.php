	...
	public function analyze( FactSet $facts ): ?Finding {
		...
		return $this->recommend(
			array(
				...
				'title'    => __( 'The core update notice is shown to people who cannot update', 'hakeemify-debloater' ),
				'summary'  => sprintf(
					/* translators: 1: number of people who edited content recently, 2: number of administrators. */
					__( '%1$d people edited content here in the last week and %2$d of the accounts are administrators.', 'hakeemify-debloater' ),
					$editors,
					$admins
				),
				'why'      => __(
				...
				'evidence' => $this->evidence( $facts )
					->fact( __( 'Core update notice registered', 'hakeemify-debloater' ), 'admin.update_nag' )
					->fact( __( 'Administrators', 'hakeemify-debloater' ), 'users.admin_count' )
					->optional( __( 'People who edited content in the last week', 'hakeemify-debloater' ), 'users.recent_editors_7d' )
					->build(),
			)
		);
	}
